...
//    private val context = getApplication<Application>()
    private val levelWinRoomVM = LevelWinRoomViewModel(context)
    private val rankingServerVM = RankingServerViewModel(context)
    private val levelRoomVM = LevelRoomViewModel(context)

//    private val roomList: List<LevelWin> = levelWinRoomVM.getAllLevelWins()
//    private val serverList: List<LevelWin> = rankingServerVM.getLevelWins()
...
//                rankingServerVM.postListLevelWin(roomList, userDataStore.getUser())
    ...
    fun upDateLevelWins() {
        infoLog("RankVM::upDateLevelWins", "start")
        viewModelScope.launch(Dispatchers.IO) {
            upDateServerLevelWinFromRoom()
            upDateLevelWinRoomFromServer()
//            updateLevelsSolution()
        }
    }
    ...
    fun upDateLevelWinRoomFromServer() {
        ...
        if (roomList.containsNotAll(serverList)) {
            ...
            levelRoomVM.addLevelSolutions(listToAdd)
        }
    }

    private fun updateLevelsSolution() {
        infoLog("RankVM::upDateLevelSolution", "start")
        val roomList: List<LevelWin> = levelWinRoomVM.getAllLevelWins()
        val serverList: List<LevelWin> = rankingServerVM.getLevelWins()

        if (roomList.containsNotAll(serverList)) {
            val listToAdd: List<LevelWin> = serverList.filterNot { roomList.contains(it) }
            levelRoomVM.addLevelSolutions(listToAdd)
        }
    }
}